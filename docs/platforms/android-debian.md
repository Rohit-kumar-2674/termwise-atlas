...
Type `exit` to return to Termux. Run `proot-distro login debian` to resume. Back up from the outer shell using the backup options printed by your installed `proot-distro --help`; review the destination and available space first.
